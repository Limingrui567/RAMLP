...
output = torch.load(path_output).detach().to(device).to(torch.float32)[:,3].reshape(-1, 1)
...
checkpoint_file = "checkpoint_MHP_CP"
...
for epoch in range(start_epoch, num_epochs):
    ...
    torch.save(model, "model_MHP_CP.pth")
    torch.save(tra_losses, "tra_losses_MHP_CP.pth")
    torch.save(val_losses, "val_losses_MHP_CP.pth")
