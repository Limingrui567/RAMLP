...
output = torch.load(path_output).detach().to(device).to(torch.float32)[:,1].reshape(-1, 1)
...
checkpoint_file = "checkpoint_MHP_V"
...
for epoch in range(start_epoch, num_epochs):
    ...
    torch.save(model, "model_MHP_V.pth")
    torch.save(tra_losses, "tra_losses_MHP_V.pth")
    torch.save(val_losses, "val_losses_MHP_V.pth")
