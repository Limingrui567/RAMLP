...
output = torch.load(path_output).detach().to(device).to(torch.float32)[:,0].reshape(-1, 1)
...
checkpoint_file = "checkpoint_MHP_U"
...
for epoch in range(start_epoch, num_epochs):
    ...
    torch.save(model, "model_MHP_U.pth")
    torch.save(tra_losses, "tra_losses_MHP_U.pth")
    torch.save(val_losses, "val_losses_MHP_U.pth")
